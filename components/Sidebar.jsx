...
// ─────────────────────────────────────────────
// Live Search Input – allows spaces while typing
// ─────────────────────────────────────────────
function SearchInput({ className = "", onResultClick }) {
      ...
      try {
        ...
        const searchItems = data
          ...
          .map((item) => ({
            ...
            category:
              item.category?.category_name?.toLowerCase().replace(/\s+/g, "-") || "news",
            date: item.news_date || "",
          }));
  ...
  function handleInput(e) {
    const value = e.target.value; // ← NO trim → spaces are kept
    setQuery(value);
    ...
    <div ref={wrapperRef} className={`relative ${className}`}>
      <div className="flex items-center bg-white rounded-md px-3 py-2 border border-gray-300">
        ...
        <input
          ...
          className="flex-1 outline-none text-gray-800 text-sm bg-white w-full"
          aria-label="Search"
        />
      </div>
...
// ─────────────────────────────────────────────
// Selected Articles – Author BEFORE Category + real author fetch
// ─────────────────────────────────────────────
    ...
    <div className="mb-6">
      ...
      <ul className="flex flex-col gap-5">
        {filtered.slice(0, 3).map((article) => {
          ...
          // Author fetch per category
          const [author, setAuthor] = useState(null);
          useEffect(() => {
            const authorId = categoryAuthorMap[article.category];
            if (!authorId) return;

            fetch(`${AUTHOR_API_BASE}${authorId}`)
              .then((res) => res.json())
              .then((data) => {
            ...
            <li key={article.slug || Math.random()}>
              <Link href={href} title={title} className="block group">
                <article>
                  ...
                  {/* Author BEFORE Category */}
                  {author ? (
                    <div className="flex items-center gap-2 text-xs mb-1">
                      <span className="font-bold text-gray-800">{author.name}</span>
                      <span className="text-gray-400">•</span>
                      <span className="uppercase text-[#d43939] font-semibold">
                        {article.category?.replace(/-/g, " ") || "News"}
                      </span>
                    </div>
                  ) : (
                    <div className="text-xs text-gray-500 mb-1">Loading author...</div>
                  )}
...
// ─────────────────────────────────────────────
// Small Thumbnail List – Author BEFORE Category
// ─────────────────────────────────────────────
                ...
                <div className="flex-1">
                  ...
                  {author ? (
                    <div className="text-xs text-gray-600 mb-1">
                      ...
                      <span className="uppercase text-[#d43939]">
                        {article.category?.replace(/-/g, " ") || "News"}
                      </span>
                    </div>
                  ) : (
...
// ─────────────────────────────────────────────
// Recent Authors Section – only 3 authors with photo, name, bio
// ─────────────────────────────────────────────
function RecentAuthors() {
  const [authors, setAuthors] = useState([]);

  useEffect(() => {
    // Fetch all 8 authors once
    const fetchAuthors = async () => {
      ...
      // Take first 3 (or randomize if you prefer recent/active)
      setAuthors(fetched.slice(0, 3));
    };
...
export default function Sidebar({
  ...
  return (
    <aside className="w-full lg:max-w-xs mx-auto lg:mx-0 lg:sticky lg:top-4">
      ...
      {/* Recent Authors Section – placed above Categories */}
      <RecentAuthors />
