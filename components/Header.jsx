...
function SearchInput({ className = "", onResultClick }) {
  ...
  // Load search index once (exclude Puerto Rico) - Force refetch to clear any old cache
  useEffect(() => {
    async function fetchAllNews() {
      try {
        setLoading(true);
        const res = await fetch(API_URL, { cache: "no-store" }); // No cache to force fresh data
        ...
        delete window.allSearchItems;
        window.allSearchItems = searchItems;

        console.log(`Search index loaded: ${searchItems.length} items (Puerto items excluded)`); // Debug log
      } catch (err) {
    ...
    // Always fetch to ensure fresh data without old cached Puerto items
    fetchAllNews();
  }, []);
...
export default function Header() {
  ...
  // Fetch breaking/latest news for ticker – exclude Puerto Rico
  useEffect(() => {
    async function fetchNews() {
      try {
        setTickerLoading(true);
        const res = await fetch(API_URL, { cache: "no-store" }); // Fresh fetch
      ...
      <div className="bg-[#d43939] text-white">
        <div className="max-w-full mx-auto px-4 md:px-[5%] py-4 md:py-5">
          ...
          <div className="hidden md:flex items-center justify-between">
            ...
            {/* Optional: desktop search toggle (uncomment if you want icon) */}
            {/* <button
        ...
        {/* Desktop search panel (if you re-enable the icon) */}
        {searchOpen && (
